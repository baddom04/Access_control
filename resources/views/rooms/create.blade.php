        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="card shadow">
                    <div class="card-body">
                        <h5 class="card-title text-center mb-4">Room Creation Form</h5>
                        <form action="{{ route('rooms.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" id="description" name="description"
                                    ...
                                    placeholder="Type something..." value="{{ old('description') }}">
                                @error('description')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                ...
                                <select id="positions" name="positions[]"
                                    ...
                                    @foreach ($positions as $position)
                                        <option value="{{ $position->id }}"
                                            @if (is_array(old('positions')) && in_array($position->id, old('positions'))) selected @endif>
                                            {{ $position->name }}
                                        </option>
                                    @endforeach
                                </select>
